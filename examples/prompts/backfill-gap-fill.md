...
Find mail without the expected provider label. Use domain dedupe where possible.
Skip threads where provider-rules.md is already satisfied (correct label on thread).
Report the plan before applying — no Gmail mutations while dry_run is true.
...
Remind me to re-import gmail-filters.xml if rule count changed.
```
